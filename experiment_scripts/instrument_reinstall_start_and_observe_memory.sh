...
#  the second (ie NNN) is the number of methods to trace , 
#  The third is the external storage global workspace (to store datas-eg all generated files)
	
#
# usage : instrument_start_observe_memory.sh apk_path number_of_methods
# example in our case: ./instrument_start_observe_memory.sh 
#	android_frida_tools/apks/locker-fe666e209e094968d3178ecf0cf817164c26d5501ed3cd9a80da786a4a3f3dc4.apk NNN
#    /media/lavoisier/ee9567d1-e98a-411a-8cea-516195e51630/frida_evaluation_on_many_apps
# 		
#
#it will produce a folder named NNN_methods_tested in the folder
#  memory_observations/app_file_name/
#  NB: supposing that the folder $3/memory_observations/app_file_name_without_extension is present, 
#  it will create $3/memory_observations/app_file_name_without_extenxion/NNN_methods_traced if it not exist
...
json_input_file=$3/methods_loaded_by_apps/input_files_for_experiments_for_${app_file_name_without_extension}/$2_entries.json
echo "----- ----- ----- >> Json input file $json_input_file "
...
if [ -z "$1" ] 
   then
	...
	RESULT_FOLDER=$3/memory_observations/${app_file_name_without_extension}/$2_methods_tested
	...
	(adb logcat > $tracer_output_file_path) &
	###( cd android_frida_tools && ./tracer.py -t -f $json_input_file > $tracer_output_file_path ) &
	###( cd android_frida_tools && ls -ls > ../$tracer_output_file_path) &
   	pid=$(echo $!)
	pid=$[$pid+2]
	echo $pid > $pid_file
    ...
    echo "----- ----- ----- >> Third mesurements after sending js method codes "
   	...
   	old_file_size="0"
	while [ $i -lt 33 ] #33 is for Jesus Christ !!! 
	do
        ...
        if [ "$new_file_size" -eq "$old_file_size" ] && [ "$new_file_size" -ne "0" ]  ### compare the size difference before continue
		then
